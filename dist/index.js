...
var app = new Hono2();
app.use("/*", cors());
var authMiddleware = /* @__PURE__ */ __name((secret) => jwt({ secret }), "authMiddleware");
app.get("/health", (c) => {
...
app.post("/api/auth/register", async (c) => {
  try {
    ...
    ).bind(email, passwordHash, (/* @__PURE__ */ new Date()).toISOString()).first();
    if (!result) {
      throw new Error("Failed to create user");
    }
    const token = await new SignJWT({ userId: result.id, email }).setProtectedHeader({ alg: "HS256" }).setExpirationTime("7d").sign(new TextEncoder().encode(c.env.JWT_SECRET));
    return c.json({
      success: true,
      token,
...
app.post("/api/auth/login", async (c) => {
  try {
    ...
    const token = await new SignJWT({ userId: user.id, email: user.email }).setProtectedHeader({ alg: "HS256" }).setExpirationTime("7d").sign(new TextEncoder().encode(c.env.JWT_SECRET));
    return c.json({
      success: true,
      token,
...
app.post("/api/todos", async (c) => {
  try {
    ...
    ).bind(payload.userId, title.trim(), 0, (/* @__PURE__ */ new Date()).toISOString()).first();
    if (!result) {
